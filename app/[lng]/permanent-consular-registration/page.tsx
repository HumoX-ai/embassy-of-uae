...
export async function generateMetadata({
  ...
  const titles = {
    en: "Permanent Consular Registration | Embassy of Uzbekistan in UAE",
    uz: "Doimiy konsullik hisobiga olish | O'zbekiston Elchixonasi BAA",
  };

  const descriptions = {
    en: "Learn about the procedure for permanent consular registration for citizens of Uzbekistan abroad.",
    uz: "Xorijda bo'lgan O'zbekiston fuqarolari uchun doimiy konsullik hisobiga olish tartibi haqida bilib oling.",
  };
  ...
  return {
    ...
    keywords:
      validLng === "uz"
        ? "doimiy konsullik, hisobga olish, O'zbekiston fuqarolari"
        : "permanent consular, registration, Uzbekistan citizens",
    openGraph: {
      title: titles[validLng] || titles.en,
      description: descriptions[validLng] || descriptions.en,
      type: "website",
    ...
    alternates: {
      canonical: `https://uzembassy.ae/${validLng}/permanent-consular-registration`,
    },
  };
}

export default async function PermanentConsularRegistrationPage({
  params,
}: PageProps) {
  const { lng } = await params;
  const validLng = languages.includes(lng as Language)
    ? (lng as Language)
    : fallbackLng;
  const { t } = await useTranslation(validLng, "permanentConsularRegistration");
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="prose prose-base md:prose-lg max-w-none text-foreground leading-relaxed space-y-6">
              ...
              <ul className="list-disc list-inside space-y-2 ml-4 text-sm md:text-base lg:text-lg">
