...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const response = await fetch('https://api.uzembassy.ae/api/news/all?limit=100', {
    ...
    if (response.ok) {
      ...
      languages.forEach(lng => {
        newsArticles.forEach((article: { id: string | number; updated_at?: string }) => {
          staticUrls.push({
            ...
            alternates: {
              languages: Object.fromEntries(
                languages.map(l => [l, `${baseUrl}/${l}/news/${article.id}`])
              ),
            },
          })
        })
